...
export class CreepDemandTaskRepo extends BaseRepo<Task> implements Persistent {

    constructor(log: Logger) { super('creep_demand', log); }
    ...
    restore(): void {
        if (Memory.persistency?.hasOwnProperty(this.key))
            this.tasks = Memory.persistency.creep_demand;
    }

    save(): void {
        this.mergeEmpty();
        Memory.persistency = Object.assign(Memory.persistency, { creep_demand: this.tasks ?? [] });
    }
...
profiler.registerClass(CreepDemandTaskRepo, 'CreepDemandTaskRepo');
