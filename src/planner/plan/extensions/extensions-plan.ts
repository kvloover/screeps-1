...
export class ExtensionsPlan implements IPlan {
    ...
    create: PlanCreateFn = (roomName, poi, terrain): StructurePlan[][] => {
        ...
        let extensions = 0;
        while (extensions < 50) {
            const dt = distanceTransform(roomName, terrain, distanceType.Manhattan, false, CUTOFF_WALKABLE, { x1: 2, y1: 2, x2: 47, y2: 47 }); // do not include roads
            ...
            while (!center && index < sorted.length) {
                ...
                // this.log.debug(roomName, `ExtensionPlan: border seeds for center ${center.x},${center.y} size ${size}: ${borderStamps.map(x => `${x.x},${x.y}`).join(';')}`);
    ...
    private borderStamp(center: Point, size: number): Point[] {
        ...
        points.push({ x: center.x, y: center.y + size });
        points.push({ x: center.x, y: center.y - size });
        points.push({ x: center.x - size, y: center.y });
        points.push({ x: center.x + size, y: center.y });
